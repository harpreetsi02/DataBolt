export function transformQuery(query) {
  ...
  const numericColumns = ["price", "salary", "total"];

  numericColumns.forEach((col) => {
    // replace: price < 1000 → CAST(price AS INTEGER) < 1000
    const regex = new RegExp(`\\b${col}\\b\\s*([<>=]+)\\s*(\\d+)`, "gi");

    q = q.replace(regex, `CAST(${col} AS INTEGER) $1 $2`);
  });
